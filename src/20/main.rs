...
fn main() {
    let input = fs::read_to_string("src/20/input.txt").unwrap();
...
fn mix(input: &str) -> i32 {
    ...
    for n in original.iter() {
        ...
        let size = mixed.len() as i32;
        let new_index = (old_index as i32 + n.1).rem_euclid(size) as usize;
